...
# Uses the stored influence data directly (no recomputation needed)
...
# Load the feature token influence data (influence is already computed!)
...
# Load tokenizer for token decoding (optional - only if showing example tokens)
if SHOW_EXAMPLE_TOKENS:
    print(f"\nLoading tokenizer for token decoding...")
    tokenizer = AutoTokenizer.from_pretrained("EleutherAI/pythia-70m-deduped")
    
    # Load text data to get example tokens
    DATA_FILE = Path("wikitext-2-train.txt")
    if DATA_FILE.exists():
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            text = f.read()
        all_tokens = tokenizer(text, return_tensors="pt")["input_ids"][0]
        print(f"Loaded {len(all_tokens)} tokens for example decoding")
        
        # Get a sample batch for token display (just for illustration)
        max_start = len(all_tokens) - BATCH_SIZE
        sample_start = random.randint(0, max_start)
        sample_tokens = all_tokens[sample_start:sample_start + BATCH_SIZE]
        print(f"Using sample batch starting at position {sample_start} for token examples")
    else:
        print(f"Warning: {DATA_FILE} not found. Cannot show example tokens.")
        SHOW_EXAMPLE_TOKENS = False
        sample_tokens = None
else:
    sample_tokens = None
...
print(f"{'='*80}")
print(f"\nUsing stored influence data (no recomputation needed)")
...
for batch_idx in batch_indices:
    ...
    print(f"\n{'='*80}")
    print(f"Batch {batch_idx} from stored data (Sequence length: {seq_len})")
    print(f"{'='*80}")
    ...
    print(f"{'Position':<10} | {'Influence':<12} | {'Normalized':<12} | {'Token Text (example)'}")
    print("-" * 90)
    ...
    for pos_idx in top_k_indices:
        ...
        # Show example token if available (note: this is just an example, not the exact token)
        if SHOW_EXAMPLE_TOKENS and sample_tokens is not None and pos_idx < len(sample_tokens):
            token_id = int(sample_tokens[pos_idx].item())
    ...
    plt.title(f'Feature {FEATURE_IDX} - Batch {batch_idx} (from stored data)\nInfluence Distribution', 
...
if SHOW_EXAMPLE_TOKENS:
    print(f"- Note: Token text shown is from example batches for illustration only")
    print(f"  (exact tokens not stored in data file)")
print(f"{'='*80}")
