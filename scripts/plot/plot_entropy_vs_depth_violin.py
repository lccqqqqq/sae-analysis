...
(the run with the most layer files; ties broken by mtime).

Output:
    figures/entropy_vs_depth__<preset>.png    (one file per preset)

Usage:
    python scripts/plot/plot_entropy_vs_depth_violin.py --preset gpt2-small
    python scripts/plot/plot_entropy_vs_depth_violin.py --preset pythia-70m llama-3.2-1b
    python scripts/plot/plot_entropy_vs_depth_violin.py --all   # all presets with data
"""
...
def load_preset(preset, run_timestamp=None):
    ...
    if d is None:
        raise FileNotFoundError(
            f"Found files in {run_dir} but none matched layer<N>.pt"
        )
    meta = {
        "preset": preset,
        "run_dir": str(run_dir),
        "num_batches": len(d["batch_results"]),
...
def plot_one(preset, run_timestamp=None):
    ...
    out = FIGURES_DIR / f"entropy_vs_depth__{preset}.png"
    fig.tight_layout()
